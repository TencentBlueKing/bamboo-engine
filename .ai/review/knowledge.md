...
这是当前 LTS 的源码索引和契约，不是缺陷清单。2026-09-09 刷新远程基线：`8ee03f25d920c8802510f41b31674b9793fcfba3`。AI CI 原接入基线为 master 的 `30438665e61945b90bf4c6789db94e7cf42f31d4`；本次包含 PR #290 中尚待合入的长输出与目标分支接入检测修复。默认 master 的工作流决定全仓触发入口，再显式读取 PR 目标 base 的脚本和知识；本 LTS 工作流副本供维护回移，单独合入该副本不会改变入口。业务源码事实以本分支为准；以后审查需再次读取实际 PR base/head。
