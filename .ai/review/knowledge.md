# bamboo_pipeline_3.29_lts 审查知识库

这是当前 LTS 的源码索引和契约，不是缺陷清单。2026-09-09 刷新远程基线：`cd9f7b80a340f89bd66711bbf8007dc23411f397`。AI CI 原接入基线为 master 的 `30438665e61945b90bf4c6789db94e7cf42f31d4`；本次包含 PR #290 中尚待合入的长输出与目标分支接入检测修复。默认 master 的工作流决定全仓触发入口，再显式读取 PR 目标 base 的脚本和知识；本 LTS 工作流副本供维护回移，单独合入该副本不会改变入口。业务源码事实以本分支为准；以后审查需再次读取实际 PR base/head。

## 1. 发行包与分支边界

| 范围 | 当前版本/声明 | 入口 |
| --- | --- | --- |
| 核心 bamboo-engine | `2.11.3`；Python `>=3.6,<4` | 根 `pyproject.toml`、`bamboo_engine/__version__.py` |
| Django/Celery 运行时 bamboo-pipeline | `3.29.9`；engine 依赖 `^2.11.3` | `runtime/bamboo-pipeline/pyproject.toml`、`runtime/bamboo-pipeline/pipeline/__init__.py`、`runtime/bamboo-pipeline/poetry.lock` |
...
3.29 有 LOOP_READY、loop_enabled/loop_fail_skip、loop_outputs_key 和循环输出追加，Context/ServiceWrapper 会传递 inner_loop；同时包含从指定主干节点开始执行和可选 rollback。它仍不含 master 的 SubCanvas。
...
本分支争锁失败时仅 MULTIPLE_CALLBACK 进入随机延迟重投路径；不能借用 3.24 新增 callback_lock_retryable 单次回调重试协议。Engine 和 ServiceActivityHandler 的 HookType/hook_dispatch 需按成功、失败、异常及恢复路径保持触发语义。
...
循环节点由 `bamboo_engine/eri/models/node.py` 的 next_node_id_in_loop 控制；Context.extract_outputs 接受 node，对 loop_outputs_key 追加结果并含 result/inner_loop。loop、inner_loop、retry 不可互换，检查失败跳过、子流程重入、并行共享变量和恢复后重复追加。计算变量/组件的 inner_loop 签名也需兼容运行时实现。
...
核心 Settings 默认 MAKO_TEMPLATE_NAME_WHITELIST_MODE=enforce，旧表达式 MAKO_SAFETY_CHECK 默认 True。两套 checker 已校验 AST、表达式/标签过滤器、危险名称/属性/下标及 format/format_map；名称白名单 warn 仅记录，off 不做该白名单。配置导入模块、extra whitelist 和可调用对象会扩大能力边界。tests/template/test_template.py 及旧 test_expression 已有过滤器副作用、白名单、允许表达式和 format 回归；不能断言这些保护不存在，也不能把有限测试通过当完整安全。

本分支没有 render_backend.py/render_transport.py；实际 Mako render 与组件执行位于宿主调用链内。sandbox 组织屏蔽名与导入模块，不提供 OS 子进程池、网络隔离或资源限额。完整变量引用与可选 ENABLE_RENDER_OBJ_BY_MAKO_STRING 的嵌套对象取值要保持类型；异常/禁止模板通常保留原文。
...
rollback 位于 `runtime/bamboo-pipeline/pipeline/contrib/rollback/`，默认 PIPELINE_ENABLE_ROLLBACK=False，需安装 app、迁移和配置 worker/队列。TOKEN 沿受 token 限制路径执行回退；ANY 跳转/清理执行信息而不调用节点 rollback，边界见该分支 api/handler/graph 与 `docs/user_guide/rollback.md`。node_timeout、node_timer_event、plugin_execute 也为可选扩展；涉及结束信号/预约回滚时检查版本和重复触发。该分支没有 3.24 的 reliable_events 或 diagnostics 模块。
...
当前业务 CI 配置：Python 3.6/3.7，Django 矩阵标签 2.2/3.0，ubuntu-20.04，actions/setup-python@v2。运行时安装命令使用 `Django>矩阵值`，标签不是最终安装版本。`.github/workflows/pr_check.yml` 的目标分支过滤为 master、develop。该过滤不包含当前 LTS，因此不能声称旧业务 PR 检查会自动运行。 新增 AI workflow 独立使用无分支过滤的 pull_request_target；接入本 LTS 并不自动修复/扩大旧业务 CI，也不证明其历史 runner 仍可用。按运行记录区分未触发、等待 runner、跳过、执行失败和通过。

## 6. 版本、发行与审查证据

核心版本同步根 pyproject 与 `bamboo_engine/__version__.py`；运行时同步自身 pyproject 与 `pipeline/__init__.py`，依赖变更核对 runtime 的 poetry.lock。两包分别通过 `.github/workflows/engine_python_package_poetry.yml` 与 `.github/workflows/runtime_pipeline_python_package_poetry.yml` 构建，tag 模式分别是 `bamboo-engine-v*.*.*` 和 `bamboo-pipeline-v*.*.*`。确认本分支解释器、构建目录与最终包元数据，不套用 master 的发布步骤。本分支有 `docs/release_process.md`，其内容仍须与本分支实际发行 workflow 对照。
