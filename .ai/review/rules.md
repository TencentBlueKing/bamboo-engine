# bamboo_pipeline_4.0_lts AI 审查约束
...
- 核心/运行时要求 Python >=3.11,<3.12；pipeline 4.0.4 精确依赖 engine 3.0.5，Django >=4.2,<5、Celery >=5.2,<6、Mako ^1.3。核对目标解释器、AST 与 Mako 版本，不机械要求 Python 3.6，也不引入 Python 3.12 专属行为。
- 存在 loop/inner_loop 重入字段，但没有 LOOP_READY、loop_enabled/loop_fail_skip、循环输出列表或 3.29 的 Context/组件 inner_loop 扩展签名；输出提取只有普通映射。不能照搬 master 节点循环协议。
- 有主干起点校验、HookType/hook_dispatch、可选 rollback/timeout/timer/plugin_execute 和 diagnostics；保持 TOKEN/ANY、信号/钩子副作用、管理权限和配置未启用的边界。没有 3.24 的 reliable_events、callback_lock_retryable 或 Mako 子进程 backend。
- Mako 默认白名单 enforce，旧安全检查默认 True；已有过滤器/format 等回归，off/warn/enforce 边界不同，测试中记录了 whitelist off 的 self/module 行为。不得把默认配置与宿主实际配置混同，也不据历史绕过样例断言本次 PR 有新增漏洞。
- 现有 pr_check 的 master/develop/python_upgrade_master 过滤不包含 bamboo_pipeline_4.0_lts；新增 AI 不补齐旧业务 CI，测试触发与通过要单独核验。
