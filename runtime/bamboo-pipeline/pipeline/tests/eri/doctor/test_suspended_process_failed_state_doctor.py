...
from pipeline.eri.doctor import SuspendedProcessFailedStateDoctor


class SuspendedProcessFailedStateDoctorTestCase(TestCase):
    def setUp(self) -> None:
        self.doctor = SuspendedProcessFailedStateDoctor(
            MagicMock(id=1, current_node_id="n1", root_pipeline_id="root_pipeline", pipeline_stack='["root_pipeline"]'),
            MagicMock(node_id="n1"),
        )

    def test_advice(self):
        self.assertEqual(self.doctor.advice(), "current node is failed, make process asleep")
