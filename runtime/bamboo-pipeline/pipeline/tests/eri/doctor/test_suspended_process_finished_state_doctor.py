...
from pipeline.eri.doctor import SuspendedProcessFinishedStateDoctor


class SuspendedProcessFinishedStateDoctorTestCase(TestCase):
    def setUp(self) -> None:
        self.doctor = SuspendedProcessFinishedStateDoctor(
            MagicMock(id=1, current_node_id="n1", root_pipeline_id="root_pipeline", pipeline_stack='["root_pipeline"]'),
            MagicMock(node_id="n1"),
        )

    def test_advice__has_suspended_stack(self):
        runtime = MagicMock()
        runtime.batch_get_state_name = MagicMock(return_value={"root_pipeline": "SUSPENDED"})
        ...
        runtime.batch_get_state_name.assert_called_once_with(["root_pipeline"])

    def test_advice__current_node_does_not_exist(self):
        runtime = MagicMock()
        runtime.batch_get_state_name = MagicMock(return_value={"root_pipeline": "RUNNING"})
    ...
    def test_advice__current_node_is_parallel_gateway(self):
        ...
        with patch("pipeline.eri.doctor.BambooDjangoRuntime", MockBambooDjangoRuntime):
            self.assertEqual(self.doctor.advice(), "set process state to sleep")
    ...
    def test_heal__has_suspended_stack(self):
        runtime = MagicMock()
        runtime.batch_get_state_name = MagicMock(return_value={"root_pipeline": "SUSPENDED"})
        MockBambooDjangoRuntime = MagicMock(return_value=runtime)
