...
class DiagnosticRulesTestCase(TransactionTestCase):
    ...
    def test_old_schedule_count_cannot_hide_current_callback_conflict(self):
        process = self._create_process(150)
        self._create_state("node-rules", version="v2")
        self._create_schedule(150, process.id, "node-rules", version="v1", schedule_times=100, expired=True)
        current = self._create_schedule(151, process.id, "node-rules", version="v2", schedule_times=0)
        callback = self._create_callback_data("node-rules", version="v2", callback_data_id=150)

        hits = diagnose_snapshot(self._snapshot())

        self.assertEqual([hit.type for hit in hits], ["callback_lock_conflict"])
        self.assertEqual(hits[0].related_objects["schedule_ids"], [current.id])
        self.assertEqual(hits[0].related_objects["callback_data_ids"], [callback.id])

    def test_partial_snapshot_does_not_infer_missing_children(self):
        parent = self._create_process(151, node_id="gateway", need_ack=2, ack_num=0)
        self._create_process(152, node_id="child", parent_id=parent.id)
        self._create_state("gateway", name=states.FINISHED)
        self._create_state("child")

        for snapshot in (self._snapshot(node_id="gateway"), self._snapshot(node_id="", process_id=parent.id)):
            with self.subTest(process_id=snapshot.process_id):
                self.assertEqual(diagnose_snapshot(snapshot), [])

    def test_old_schedule_does_not_satisfy_current_running_version(self):
        process = self._create_process(153)
        self._create_state("node-rules", version="v2")
        self._create_schedule(153, process.id, "node-rules", version="v1", expired=True)

        hits = diagnose_snapshot(self._snapshot(), stall_seconds=3600)

        self.assertEqual([hit.type for hit in hits], ["schedule_missing_for_running_node"])
        self.assertEqual(hits[0].evidence["version"], "v2")
