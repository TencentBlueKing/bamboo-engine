...
class ProgressTest(TransactionTestCase):
    ...
    def test_silent_window_includes_floor_and_excludes_threshold(self):
        now = timezone.now()
        _proc("at-floor", beat=now - timedelta(seconds=7200))
        _proc("before-floor", beat=now - timedelta(seconds=7201))
        _proc("at-threshold", beat=now - timedelta(seconds=1800))
        _proc("past-threshold", beat=now - timedelta(seconds=1801))

        roots = [r for r, _ in progress.stalled_root_candidates(1800, 100, now=now, max_silent_seconds=7200)]

        self.assertEqual(roots, ["past-threshold", "at-floor"])

    def test_silent_bound_preserves_max_across_live_branches(self):
        now = timezone.now()
        _proc("stalled", beat=now - timedelta(days=400), node="old")
        _proc("stalled", beat=now - timedelta(seconds=3600), node="recent")
        _proc("stalled", dead=True, beat=now, node="dead")
        _proc("active", beat=now - timedelta(days=400), node="old")
        _proc("active", beat=now - timedelta(seconds=3600), node="stale")
        _proc("active", beat=now, node="fresh")

        candidates = progress.stalled_root_candidates(1800, 100, now=now, max_silent_seconds=7200)

        self.assertEqual(candidates, [("stalled", now - timedelta(seconds=3600))])
