...
def test_failed_node_process_not_asleep_still_reported():
    ...
    assert [hit.type for hit in diagnose_snapshot(snapshot, stall_seconds=7200)] == ["process_alive_but_terminal_state"]
...
def _gateway_with_failed_branches_snapshot(live_children_failed=2, dead_children=2, ack_num=2, need_ack=4):
    ...
    for index in range(live_children_failed):
        ...
        states.append(types.SimpleNamespace(id=10 + index, node_id=node, name=engine_states.FAILED, version="v1"))
...
def test_no_fallback_when_suspended_before_state_creation():
    snapshot = _parked_snapshot(suspended=True)
    snapshot.states.clear()
    assert diagnose_snapshot(snapshot, stall_seconds=7200) == []


def test_partial_parking_does_not_hide_poll_stall():
    snapshot = _parked_snapshot(frozen=True)
    polling = _waiting_snapshot(ScheduleType.POLL, node="polling")
    polling.processes[0].id = 2
    polling.states[0].id = 2
    polling.schedules[0].process_id = 2
    snapshot.processes.extend(polling.processes)
    snapshot.states.extend(polling.states)
    snapshot.schedules.extend(polling.schedules)
...
def test_no_fallback_when_gateway_children_are_suspended_or_frozen():
    snapshot = _gateway_with_failed_branches_snapshot()
    snapshot.states[1].name = engine_states.RUNNING
    snapshot.states[2].name = engine_states.RUNNING
    snapshot.processes[1].suspended = True
    snapshot.processes[2].frozen = True

    assert diagnose_snapshot(snapshot, stall_seconds=7200) == []


def test_dead_process_does_not_count_as_live_parking():
    snapshot = _parked_snapshot(suspended=True)
    snapshot.processes[0].dead = True
