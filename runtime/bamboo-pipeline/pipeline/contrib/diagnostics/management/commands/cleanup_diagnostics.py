...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        case_queryset = DiagnosticCase.objects.exclude(status=DiagnosticCase.STATUS_OPEN).filter(
            updated_at__lt=now - datetime.timedelta(days=conf.case_retention_days())
        )
